"""
dashboard_builder.py - ナレッジデータからHTMLダッシュボードを生成
"""
...
def build_html(days_data, proposals):
    ...
    # タグHTML
    tag_html = "".join([
        f'<span class="tag">{tag} <span class="tag-count">{count}</span></span>'
    ...
    for item in items[:20]:
        ...
        tags_html = "".join([f'<span class="item-tag">{t}</span>' for t in item.get("tags", [])[:3]])
        cards_html += f"""
        <div class="card" onclick="window.open('{item.get('url','')}','_blank')">
          <div class="card-importance" style="background:{bar_color};width:{importance*10}%"></div>
          <div class="card-source">{item.get('source','')}</div>
          <div class="card-title">{item.get('title_ja', item.get('title',''))}</div>
          <div class="card-summary">{item.get('summary_ja','')}</div>
          <div class="card-tags">{tags_html}</div>
          <div class="card-score">🔥 {importance}/10</div>
        </div>"""
    ...
    for d in days_data:
        ...
        history_html += f'<div class="hist-bar" style="height:{min(count*2,60)}px" title="{d.get(\"date\",\"\")} ({count}件)"></div>'
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .card-importance {{
    ...
    height: 2px; transition: width 0.3s;
  }}
...
<div class="container">
  ...
  <header>
    ...
    <div class="date">
      <span class="pulse"></span> {today} 自動更新
    </div>
  </header>
  ...
  <div class="main-grid">
    <div class="left">
      ...
      <div class="panel">
        <div class="panel-title">// Articles ({len(items)} collected)</div>
    ...
    <div class="right">
      ...
      <div class="panel">
        ...
        <div class="stat-grid">
          ...
          <div class="stat">
            <div class="stat-value">{len(days_data)}</div>
...
def main():
    print(f"🧠 Dashboard Builder starting...")
